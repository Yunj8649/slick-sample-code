jQuery(document).ready(function() {
    ...
    var swiper01_bottom = new Swiper('.main_banner_bottom', {
        ...
        pagination: false,
		speed: speedMS,
        allowTouchMove: false,	// 마우스드래그로 slide 넘기기 true / false
        on: {
            slideChangeTransitionEnd: function () {
                console.log('slideChangeTransitionEnd');
                jQuery('.main_banner_top .fadeInUp').addClass('before_hidden');
                jQuery('.main_banner_top .fadeInUp').removeClass('fadeInUp');
                jQuery('.main_banner_top .swiper-slide-active .txt_box').addClass('fadeInUp');
                jQuery('.main_banner_top .swiper-slide-active .txt_box').removeClass('before_hidden');
            },
        }
	});
    var swiper01_top = new Swiper('.main_banner_top', {
        ...
        on: {
            slideNextTransitionEnd: function(e) {
                console.log("main_banner_top :: slideNextTransitionEnd()");
                swiper01_bottom.slidePrev(speedMS);
            },
            slidePrevTransitionEnd: function(e) {
                console.log("main_banner_top :: slidePrevTransitionEnd()");
                swiper01_bottom.slideNext(speedMS);
            }
        }
	});
    var swiper01 = new Swiper('.main_banner_left', {
        ...
        on: {
            ...
            realIndexChange: function () {
                ...
                console.log(i);
                jQuery('.swiper-pagination1').html("<span>" + i + "</span>" + " / " + 3);
                setTimeout(() => {swiper01_top.slideTo(this.realIndex, 1000, false)}, 500);
                setTimeout(() => {swiper01_bottom.slideTo(this.realIndex, 1000, false)}, 1000);
            }
            */
            // 다음 슬라이드 이동 전 발생하는 이벤트
            slideNextTransitionStart: function() {
                // page number 보여주기
                var i = this.realIndex + 1;
                jQuery('.swiper-pagination1').html("<span>" + i + "</span>" + " / " + 3);
            },
            // 이전 슬라이드 이동 전 발생하는 이벤트
            slidePrevTransitionStart: function() {
                // page number 보여주기
                var i = this.realIndex + 1;
                jQuery('.swiper-pagination1').html("<span>" + i + "</span>" + " / " + 3);
            },
            // 다음 슬라이드 이동 후 발생하는 이벤트
            slideNextTransitionEnd: function(e) {
                console.log("main_banner_left :: slideNextTransitionEnd()");
                swiper01_top.slideNext(speedMS);
            },
            // 이전 슬라이드 이동 후 발생하는 이벤트
            slidePrevTransitionEnd: function(e) {
                console.log("main_banner_left :: slidePrevTransitionEnd()");
                var i = this.realIndex + 1;
                jQuery('.swiper-pagination1').html("<span>" + i + "</span>" + " / " + 3);
                swiper01_top.slidePrev(speedMS);
            }
        }
	});
